defmodule Honyaku.External.Translate.Gemini.Flash2 do
  @moduledoc """
  封装 Gemini API 的调用
  """
  ...
  def translate(text, target_lang, source_lang) do
    ...
    case Req.post(
      ...
      {:ok, reason} ->
        Logger.debug("Gemini Flash 2 Translator API调用失败，未知错误：#{inspect(reason)}")
        {:error, :unknown_error}

      {:error, reason} ->
        Logger.debug("Gemini Flash 2 Translator API调用失败：#{inspect(reason)}")
        {:error, reason}
    end
  end
end
